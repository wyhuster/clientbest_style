...
abstract class AbstractPressModel implements PressModel{
	...
	public static function reload($id)
	{
        ...
        {
               global $db; 
               $sql = "select history_record.id,history_record.desc_info,history_record.time,data_playback.type,data_playback.module_server,data_playback.press_server,data_playback.press_mode,data_playback.press_args,data_playback.tool_name,data_playback.tool_args from history_record,data_playback where history_record.data_id=data_playback.id order by history_record.id asc";
	       $result = mysql_query($sql,$db);
	       $all_models = array();
               if(!$result)
               {
